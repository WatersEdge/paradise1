...
public class L1BuffUtil {
	...
	/** 龙之血痕 */
	public static void bloodstain(final L1PcInstance pc, final byte type, final int time, final boolean showGfx) {
	...
	/** 卡瑞、莎尔的祝福 */
	public static void effectBlessOfDragonSlayer(final L1PcInstance pc, final int skillId, final int time, final int showGfx) {
		if (showGfx != 0) {
	...
	/** 设置技能效果 */
	public static int skillEffect(final L1Character _user, final L1Character cha, L1Character _target, final int skillId, final int _getBuffIconDuration, final int dmg) {
		...
		switch (skillId) {
				...
				if (cha instanceof L1PcInstance) {
					...
					if (_player.getId() != pc.getId()) {
						if (L1World.getInstance().getVisiblePlayer(pc, 0).size() > 0) {
							for (final L1PcInstance visiblePc : L1World.getInstance().getVisiblePlayer(pc, 0)) {
								if (!visiblePc.isDead()) {
			...
			case TRIPLE_ARROW:
				...
				final int[] BowGFX = { 37, 138, 3860, 3126, 3420, 2284, 3105, 3145, 3148, 3151, 3871, 4125, 2323, 3892, 3895, 3898, 3901, 4917, 4918, 4919, 4950, 6087, 6140, 6145, 6150, 6155, 6160, 6269, 6272, 6275, 6278, 6826, 6827, 6836, 6837, 6846, 6847, 6856, 6857, 6866,
						6867, 6876, 6877, 6886, 6887, 8719 };
				...
				if (cha instanceof L1MonsterInstance) {
					...
					if (npc.getNpcTemplate().isTamable()) {
						...
						if (charisma >= 6) { // 确保宠物的成本
							final L1SummonInstance summon = new L1SummonInstance(npc, _user, false);
							_target = summon; // 更换目标
						}
						else {
			...
			case CREATE_ZOMBIE:
				if (cha instanceof L1MonsterInstance) {
					...
					charisma -= petcost;
					if (charisma >= 6) { // 确保宠物的成本
						final L1SummonInstance summon = new L1SummonInstance(npc, _user, true);
						_target = summon; // 更换目标
					}
					else {
						_player.sendPackets(new S_ServerMessage(319)); // \f1你不能拥有太多的怪物。
	...
	/** 三段加速 */
	public static void thirdSpeed(final L1PcInstance pc) {
