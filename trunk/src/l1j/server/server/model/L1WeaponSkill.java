...
public class L1WeaponSkill {
	...
	public static double getAreaSkillWeaponDamage(final L1PcInstance pc, final L1Character cha, final int weaponId) {
		...
		if (probability >= chance) {
			...
			for (final L1Object object : L1World.getInstance().getVisibleObjects(areaBase, area)) {
				if (object == null) {
					continue;
				}
	...
	/** 骰子匕首 */
	public static double getDiceDaggerDamage(final L1PcInstance pc, final L1Character cha, final L1ItemInstance weapon) {
		double dmg = 0;
		final int chance = Random.nextInt(100) + 1;
		if (2 >= chance) {
	...
	public static double getWeaponSkillDamage(final L1PcInstance pc, final L1Character cha, final int weaponId) {
		...
		if (effectId != 0) {
			...
			if (!isArrowType) {
				...
				final S_UseAttackSkill packet = new S_UseAttackSkill(pc, cha.getId(), cha.getX(), cha.getY(), data, false);
				pc.sendPackets(packet);
				pc.broadcastPacket(packet);
			}
		}
		...
		if ((area > 0) || (area == -1)) { // 范围の场合
			for (final L1Object object : L1World.getInstance().getVisibleObjects(cha, area)) {
				...
				final int castleId = L1CastleLocation.getCastleIdByArea((L1Character) object);
				if (castleId > 0) {
