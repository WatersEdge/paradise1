...
public class L1Trade {
	...
	public void TradeAddItem(final L1PcInstance player, final int itemid, int itemcount) {
		final L1PcInstance trading_partner = (L1PcInstance) L1World.getInstance().findObject(player.getTradeID());
		final L1ItemInstance l1iteminstance = player.getInventory().getItem(itemid);
	...
	public void TradeCancel(final L1PcInstance player) {
		...
		if (trading_partner != null) {
			final List<L1ItemInstance> player_tradelist = player.getTradeWindowInventory().getItems();
			final int player_tradecount = player.getTradeWindowInventory().getSize();

			final List<L1ItemInstance> trading_partner_tradelist = trading_partner.getTradeWindowInventory().getItems();
			final int trading_partner_tradecount = trading_partner.getTradeWindowInventory().getSize();
	...
	public void TradeOK(final L1PcInstance player) {
		int cnt;
		final L1PcInstance trading_partner = (L1PcInstance) L1World.getInstance().findObject(player.getTradeID());
		if (trading_partner != null) {
			final List<L1ItemInstance> player_tradelist = player.getTradeWindowInventory().getItems();
