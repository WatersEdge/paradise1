...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		switch (itemId) {
			case 40088: case 40096: case 49308: case 140088: // 变形卷轴
				s = readS();
				break;
			case L1ItemId.SCROLL_OF_ENCHANT_WEAPON:		// 对武器施法的卷轴
			case L1ItemId.B_SCROLL_OF_ENCHANT_WEAPON:	// 受祝福的 对武器施法的卷轴
			case L1ItemId.C_SCROLL_OF_ENCHANT_WEAPON:	// 受诅咒的 对武器施法的卷轴
			case L1ItemId.SCROLL_OF_ENCHANT_ARMOR:		// 对盔甲施法的卷轴
			case L1ItemId.B_SCROLL_OF_ENCHANT_ARMOR:	// 受祝福的 对盔甲施法的卷轴
			case L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR:	// 受诅咒的 对盔甲施法的卷轴
			case L1ItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON: // 试炼卷轴
			...
			default:
				...
				else if ((use_type == 5) || (use_type == 17)) { // spell_long(魔杖)、spell_short(吸血吸魔魔法卷轴)
					spellsc_objid = readD();
					spellsc_x = readH();
					spellsc_y = readH();
				}
		...
		if (pc.getCurrentHp() > 0) {
			int delay_id = 0;
			if (l1iteminstance.getItem().getType2() == 0) { // 种别：其他项目
			...
			_log.finest("要求使用道具 (obj) = " + itemObjid + " 动作 = " + l + " 值 = " + s);

			// 对武器施法的卷轴
			if ((itemId == 40077 // 古代人的练金术卷轴
					) || (itemId == 40128 // 对武器施法的幻象卷轴
					) || (itemId == 40130 // 金侃的卷轴
					) || (itemId == 140130 // 受祝福的 金侃的卷轴
					) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_WEAPON // 对武器施法的卷轴
					) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON // 试炼卷轴
					) || (itemId == L1ItemId.B_SCROLL_OF_ENCHANT_WEAPON // 受祝福的 对武器施法的卷轴
					) || (itemId == L1ItemId.C_SCROLL_OF_ENCHANT_WEAPON // 受诅咒的 对武器施法的卷轴
					)) {
			...
			// 武器属性强化卷轴
			else if ((itemId == 41429 // 风之武器强化卷轴
					) || (itemId == 41430 // 地之武器强化卷轴
					) || (itemId == 41431 // 水之武器强化卷轴
					) || (itemId == 41432 // 火之武器强化卷轴
					)) {
				Enchant.scrollOfEnchantWeaponAttr(pc, l1iteminstance, l1iteminstance1, client);
			}

			// 对盔甲施法的卷轴
			else if ((itemId == 40078 // 古代人的咒术卷轴
					) || (itemId == 40127 // 对盔甲施法的幻象卷轴
					) || (itemId == 40129 // 奇安的卷轴
					) || (itemId == 140129 // 受祝福的 奇安的卷轴
					) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_ARMOR // 对盔甲施法的卷轴
					) || (itemId == L1ItemId.B_SCROLL_OF_ENCHANT_ARMOR // 受祝福的 对盔甲施法的卷轴
					) || (itemId == L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR // 受诅咒的 对盔甲施法的卷轴
					)) {
			...
			// 道具类
				...
				// treasure_box(宝箱)
				else if (l1iteminstance.getItem().getType() == 16) {
				...
				// 魔法娃娃类
				else if (l1iteminstance.getItem().getType() == 17) {
				...
				// 终极治愈药水
						...
						) || (itemId == L1ItemId.CONDENSED_POTION_OF_GREATER_HEALING // 浓缩终极体力恢复剂
						) || (itemId == 47114 // 凝聚的化合物
						) || (itemId == 49137 // 鲜奶油蛋糕
						) || (itemId == 41141 // 神秘的体力药水
						)) {
				...
				// 变形卷轴、福利变形药水
				else if ((itemId == 40088 // 变形卷轴
						) || (itemId == 40096 // 象牙塔变身卷轴
						) || (itemId == 49308 // 福利变形药水
						) || (itemId == 140088 // 受祝福的 变形卷轴
						)) {
				...
				else if ((itemId >= 47064 && itemId <= 47067)
							|| (itemId >= 47074 && itemId <= 47077)
							|| (itemId >= 47084 && itemId <= 47087)
							|| (itemId >= 47094 && itemId <= 47097)) {
					...
					if ((earing2Id >= 41173) && (41184 >= earing2Id)) {
						...
						if ((earing2Id >= (itemId + potion1)) && ((itemId + potion2) >= earing2Id)) {
							if ((Random.nextInt(99) + 1) < Config.CREATE_CHANCE_MYSTERIOUS) {
								...
								pc.sendPackets(new S_ServerMessage(160, l1iteminstance1.getName())); // \f1%0%s %2 产生激烈的 %1 光芒，但是没有任何事情发生。
								pc.getInventory().removeItem(l1iteminstance, 1);
							}
						} else {
							pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
				...
				// 召唤球之核
					...
					if ((dantesId >= 41030) && (41034 >= dantesId)) { // 各段阶召唤球
				...
				// 空的魔法卷軸(１～５)
				else if ((itemId == 40090
						) || (itemId == 40091
						) || (itemId == 40092
						) || (itemId == 40093
						) || (itemId == 40094
						)) {
					if (pc.isWizard()) { // 使用职业为魔法师
						if (((itemId == 40090) && (blanksc_skillid <= 7)) // 空的魔法卷軸(Lv1)可记录1级以下の魔法
								|| ((itemId == 40091) && (blanksc_skillid <= 15)) // 空的魔法卷軸(Lv2)可记录2级以下の魔法
								|| ((itemId == 40092) && (blanksc_skillid <= 22)) // 空的魔法卷軸(Lv3)可记录3级以下の魔法
								|| ((itemId == 40093) && (blanksc_skillid <= 31)) // 空的魔法卷軸(Lv4)可记录4级以下の魔法
								|| ((itemId == 40094) && (blanksc_skillid <= 39)) // 空的魔法卷軸(Lv5)可记录5级以下の魔法
								) {
							...
							if (spellsc != null) {
								if (pc.getInventory().checkAddItem(spellsc, 1) == L1Inventory.OK) {
									L1Skills l1skills = SkillsTable.getInstance().getTemplate(blanksc_skillid + 1); // blanksc_skillid从0开始
									...
									if (l1skills.getItemConsumeId() != 0) { // 必须材料
										if (!pc.getInventory().checkItem(l1skills.getItemConsumeId(), l1skills.getItemConsumeCount())) { // 检查必要材料
				...
				// 魔法卷轴
				else if ((((itemId >= 40859) && (itemId <= 40898)) && (itemId != 40863)) || ((itemId >= 49281) && (itemId <= 49286))) { // 40863(指定传送)是一个传送卷轴处理
					if ((spellsc_objid == pc.getId()) && (l1iteminstance.getItem().getUseType() != 30)) { // spell_buff(魔法卷轴)
				...
				// 便笺(未使用)
				else if ((itemId == 40310 // 信纸
						) || (itemId == 40730 // 圣诞卡片
						) || (itemId == 40731 // 情人节 卡片
						) || (itemId == 40732 // 白色情人节 卡片
						)) {
				...
				// 便笺(未开封)
				else if ((itemId == 49016 // 信纸
						) || (itemId == 49018 // 血盟信纸
						) || (itemId == 49020 // 圣诞卡片
						) || (itemId == 49022 // 情人节 卡片
						) || (itemId == 49024 // 白色情人节 卡片
						)) {
				...
				// 便笺(开封)
				else if ((itemId == 49017 // 信纸
						) || (itemId == 49019 // 血盟信纸
						) || (itemId == 49021 // 圣诞卡片
						) || (itemId == 49023 // 情人节 卡片
						) || (itemId == 49025 // 白色情人节 卡片
						)) {
					pc.sendPackets(new S_Letter(l1iteminstance));
				}
						...
						if (resobject instanceof L1PcInstance) {
							...
							if (L1World.getInstance().getVisiblePlayer(target, 0).size() > 0) {
								for (L1PcInstance visiblePc : L1World.getInstance().getVisiblePlayer(target, 0)) {
									if (!visiblePc.isDead()) {
										pc.sendPackets(new S_ServerMessage(592)); // \f1复活失败，因为这个位置已被占据。
										return;
									}
								}
							}
				...
				// 王族魔法书
					...
					if (pc.isCrown() || pc.isGm()) {
						if ((itemId == 40226) && (pc.getLevel() >= 15)) { // 王族技能LV1
							SpellBook4(pc, l1iteminstance, client);
						}
						else if ((itemId == 40228) && (pc.getLevel() >= 30)) { // 王族技能LV2
							SpellBook4(pc, l1iteminstance, client);
						}
						else if ((itemId == 40227) && (pc.getLevel() >= 40)) { // 王族技能LV3
							SpellBook4(pc, l1iteminstance, client);
						}
						else if (((itemId == 40231) || (itemId == 40232)) && (pc.getLevel() >= 45)) { // 王族技能LV4
							SpellBook4(pc, l1iteminstance, client);
						}
						else if ((itemId == 40230) && (pc.getLevel() >= 50)) { // 王族技能LV5
							SpellBook4(pc, l1iteminstance, client);
						}
						else if ((itemId == 40229) && (pc.getLevel() >= 55)) { // 王族技能LV6
							SpellBook4(pc, l1iteminstance, client);
						}
						else {
				...
				// 黑暗精灵水晶
					...
					if (pc.isDarkelf() || pc.isGm()) {
						if ((itemId >= 40265) && (itemId <= 40269) && (pc.getLevel() >= 15)) { // 黑暗精灵技能LV1
							SpellBook1(pc, l1iteminstance, client);
						}
						else if ((itemId >= 40270) && (itemId <= 40274) && (pc.getLevel() >= 30)) { // 黑暗精灵技能LV2
							SpellBook1(pc, l1iteminstance, client);
						}
						else if ((itemId >= 40275) && (itemId <= 40279) && (pc.getLevel() >= 45)) { // 黑暗精灵技能LV3
							SpellBook1(pc, l1iteminstance, client);
						}
						else {
							pc.sendPackets(new S_ServerMessage(312)); // 你还不能学习法术。
				...
				// 技术书
					...
					if (pc.isKnight() || pc.isGm()) {
						if ((itemId >= 40164) && (itemId <= 40165)
								&& (pc.getLevel() >= 50)) { // 冲击之晕、增幅防御
							SpellBook3(pc, l1iteminstance, client);
						}
						else if ((itemId >= 41147) && (itemId <= 41148)
								&& (pc.getLevel() >= 50)) { // 坚固防护、反击屏障
							SpellBook3(pc, l1iteminstance, client);
						}
						else if ((itemId == 40166) && (pc.getLevel() >= 60)) { // 尖刺盔甲
							SpellBook3(pc, l1iteminstance, client);
						}
						else {
							pc.sendPackets(new S_ServerMessage(312)); // 你还不能学习法术。
				...
				// 龙骑士书板
					...
					if (pc.isDragonKnight() || pc.isGm()) {
						if ((itemId >= 49102) && (itemId <= 49106 // 龙骑士技能LV1
								) && (pc.getLevel() >= 15)) {
							SpellBook5(pc, l1iteminstance, client);
						}
						else if ((itemId >= 49107) && (itemId <= 49111 // 龙骑士技能LV2
								) && (pc.getLevel() >= 30)) {
							SpellBook5(pc, l1iteminstance, client);
						}
						else if ((itemId >= 49112) && (itemId <= 49116 // 龙骑士技能LV3
								) && (pc.getLevel() >= 45)) {
							SpellBook5(pc, l1iteminstance, client);
						}
						else {
				...
				// 记忆水晶
					...
					if (pc.isIllusionist() || pc.isGm()) {
						if ((itemId >= 49117) && (itemId <= 49121 // 幻术师魔法LV1
								) && (pc.getLevel() >= 10)) {
							SpellBook6(pc, l1iteminstance, client);
						}
						else if ((itemId >= 49122) && (itemId <= 49126 // 幻术师魔法LV2
								) && (pc.getLevel() >= 20)) {
							SpellBook6(pc, l1iteminstance, client);
						}
						else if ((itemId >= 49127) && (itemId <= 49131 // 幻术师魔法LV3
								) && (pc.getLevel() >= 30)) {
							SpellBook6(pc, l1iteminstance, client);
						}
						else if ((itemId >= 49132) && (itemId <= 49136 // 幻术师魔法LV4
								) && (pc.getLevel() >= 40)) {
							SpellBook6(pc, l1iteminstance, client);
						}
						else {
				...
				// 祝福的瞬间移动卷轴、瞬间移动卷轴、象牙塔瞬间移动卷轴
				else if ((itemId == 140100
						) || (itemId == 40100
						) || (itemId == 40099
						) || (itemId == 40086 // 全体传送术卷轴
						) || (itemId == 40863 // 魔法卷轴(指定传送)
						)) {
					L1BookMark bookm = pc.getBookMark(btele);
					if (bookm != null) { // 取出一个书签
						if (pc.getMap().isEscapable() || pc.isGm()) {
							int newX = bookm.getLocX();
							int newY = bookm.getLocY();
							short mapId = bookm.getMapId();

							if (itemId == 40086) { // 全体传送术卷轴
				...
				// 各种结婚戒指
					...
					if (partner_stat) {
						...
						if (((partner.getMapId() == 0 // 说话之岛
								) || (partner.getMapId() == 4 // 整个大陆
								) || (partner.getMapId() == 304 // 沉默洞穴
								)) && (castle_area == false
								)) {
				...
				// 消灭之意志
					...
					if (pc.getQuest().get_step(L1Quest.QUEST_LEVEL50) > 0 ) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) { // 王族
							short mapid = 2000; // 古代人空间 1楼
							L1Teleport.teleport(pc, 32793, 32757, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isKnight() && (pc.getMapId() == 2001)) { // 骑士
							short mapid = 2001; // 古代人空间 2楼
							L1Teleport.teleport(pc, 32793, 32757, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isElf() && (pc.getMapId() == 2002)) { // 精灵
							short mapid = 2002; // 古代人空间 3楼
							L1Teleport.teleport(pc, 32793, 32757, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isWizard() && (pc.getMapId() == 2003)) { // 魔法师
							short mapid = 2003; // 古代人空间 4楼
							L1Teleport.teleport(pc, 32793, 32757, mapid, 5, true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else {
							pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
				...
				// 神秘贝壳
					...
					if (pc.isElf()
							...
							&& ((pc.getY() >= 32324) && (pc.getY() <= 32328)) && (pc.getMapId() == 4) // 象牙塔村南魔法阵座标
						...
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								...
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 45300) { // 古代人亡灵(试炼怪物)
										found = true;
										break;
									}
								}
				...
				// 暗杀名单(古鲁丁村)
					...
					if ((pc.getX() == 32620) && (pc.getY() == 32641) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45883, 0, 300000); // 力比(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(燃柳村)
					...
					if ((pc.getX() == 32730) && (pc.getY() == 32426) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45884, 0, 300000); // 戴度普(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(肯特村)
					...
					if ((pc.getX() == 33046) && (pc.getY() == 32806) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45885, 0, 300000); // 依卡(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(风木村)
					...
					if ((pc.getX() == 32580) && (pc.getY() == 33260) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45886, 0, 300000); // 托得(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(海音村)
					...
					if ((pc.getX() == 33447) && (pc.getY() == 33476) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45887, 0, 300000); // 罗德(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(亚丁城镇)
					...
					if ((pc.getX() == 34215) && (pc.getY() == 33195) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45888, 0, 300000); // 亚特辛(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 暗杀名单(奇岩村)
					...
					if ((pc.getX() == 33513) && (pc.getY() == 32890) && (pc.getMapId() == 4)) {
						...
						L1SpawnUtil.spawn(pc, 45889, 0, 300000); // 托达(黑妖30级任务)
					}
					else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				...
				// 松木魔杖、黑暗安特的树枝、受祝福的松木魔杖
					...
					if (pc.getMap().isUsePainwand()) {
						...
						int[] mobArray =
						{ 45008, 45140, 45016, 45021,45025, // 哥布林・哈柏哥布林・鹿・葛林
								...
								45173, 45213, 45079, 45144	// 莱肯・卡司特・冰原狼人・蜥蜴人
								};
				...
				// 肉类
				else if ((itemId == 40056) || (itemId == 40057) || (itemId == 40059 // 肉、漂浮之眼肉、蛋
						) || (itemId == 40060) || (itemId == 40061) || (itemId == 40062 // 胡萝卜、柠檬、香蕉
						) || (itemId == 40063) || (itemId == 40064) || (itemId == 40065 // 巧克力、苹果、情人礼物(糖果)
						) || (itemId == 40069) || (itemId == 40072) || (itemId == 40073 // 橘子、烤薄饼、复活蛋
						) || (itemId == 41296) || (itemId == 41297) || (itemId == 41266 // 鲷鱼、鲑鱼、蕃茄
						) || (itemId == 41267) || (itemId == 41274) || (itemId == 41275 // 起士、蚂蚁腿、熊肉
						) || (itemId == 41276) || (itemId == 41252) || (itemId == 49040 // 山猪肉、珍奇的乌龟、鲨鱼卵
						) || (itemId == 49041) || (itemId == 49042) || (itemId == 49043 // 鳄鱼肉、龙龟蛋、奇异鹦鹉肉
						) || (itemId == 49044) || (itemId == 49045) || (itemId == 49046 // 毒蝎肉、伊莱克顿肉、蜘蛛腿肉
						) || (itemId == 49047) || (itemId == 140061) || (itemId == 140062 // 蟹肉、受祝福的柠檬、受祝福的香蕉
						) || (itemId == 140065) || (itemId == 140069) || (itemId == 140072 // 受祝福的情人礼物(糖果)、受祝福的橘子、受祝福的烤薄饼
						)) {
					pc.getInventory().removeItem(l1iteminstance, 1);
					...
					pc.sendPackets(new S_ServerMessage(76, l1iteminstance.getItem().getIdentifiedNameId())); // \f1吃 %0%o。
				}

				// 进化果实
				else if (itemId == 40070) {
					pc.sendPackets(new S_ServerMessage(76, l1iteminstance.getLogName())); // \f1吃 %0%o。
					pc.getInventory().removeItem(l1iteminstance, 1);
				}
				...
				// 完成的藏宝图
					...
					if (pc.getInventory().checkItem(40621)) { // 德雷克钥匙
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
					else if (((pc.getX() >= 32856) && (pc.getX() <= 32858)) && ((pc.getY() >= 32857) && (pc.getY() <= 32858))
							&& (pc.getMapId() == 443)) { // 海贼岛地监３阶
						L1Teleport.teleport(pc, ((L1EtcItem) l1iteminstance.getItem()).get_locx(), ((L1EtcItem) l1iteminstance.getItem()).get_locy(),
								((L1EtcItem) l1iteminstance.getItem()).get_mapid(), 5, true);
					}
					else {
				...
				else if (itemId == 41226) { // 帕古的药
					pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "ei014"));
				}
				else if (itemId == 41227) { // 亚历斯的介绍书
				...
				else if (itemId == 41346) { // 罗宾孙的便条纸1
					pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "robinscroll"));
				}
				else if (itemId == 41347) { // 罗宾孙的便条纸2
				...
				else if (itemId == 49221) { // 妖魔密使首领间谍书
				...
				// 微弱的灵魂
					...
					if (((pc.getX() >= 32844) && (pc.getX() <= 32845)) && ((pc.getY() >= 32693) && (pc.getY() <= 32694)) && (pc.getMapId() == 550)) { // 船舶之墓:地上层
						L1Teleport.teleport(pc, ((L1EtcItem) l1iteminstance.getItem()).get_locx(), ((L1EtcItem) l1iteminstance.getItem()).get_locy(),
								((L1EtcItem) l1iteminstance.getItem()).get_mapid(), 5, true);
					}
					else {
				...
				// 银笛
					...
					if (((pc.getX() >= 32619) && (pc.getX() <= 32623)) && ((pc.getY() >= 33120) && (pc.getY() <= 33124))
							&& (pc.getMapId() == 440)) { // 海贼岛前半部魔法阵座标
						boolean found = false;
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								L1MonsterInstance mob = (L1MonsterInstance) obj;
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 45875) { // 海贼骷髅首领
										found = true;
										break;
									}
								}
				...
				// 火焰之影的契约书
					...
					if ((pc.getQuest().get_step(L1Quest.QUEST_SHADOWS) == L1Quest.QUEST_END) || pc.getInventory().checkItem(41122, 1)) {
						...
						createNewItem(pc, 41122, 1); // 火焰之影的契约
					}
				}
				...
				// 炎魔的契约书
					...
					if ((pc.getQuest().get_step(L1Quest.QUEST_DESIRE) == L1Quest.QUEST_END) || pc.getInventory().checkItem(41131, 1)) {
						...
						createNewItem(pc, 41131, 1); // 炎魔的契约
					}
				}
					...
					for (L1Object object : L1World.getInstance().getVisibleObjects(pc, 3)) {
						if (object instanceof L1EffectInstance) {
							if (((L1NpcInstance) object).getNpcTemplate().get_npcId() == 81170) {
								pc.sendPackets(new S_ServerMessage(1162)); // 附近已经有柴火了。
								return;
							}
						}
					}
				...
				// 魔法料理、象牙塔妙药
						...
						|| itemId == L1ItemId.POTION_OF_WONDER_DRUG
						) {
				...
				// 龟裂之核
					...
					if ((targetItemId == 49095 // 上锁的欧西里斯初级宝箱
							) || (targetItemId == 49099 // 上锁的欧西里斯高级宝箱
							) || (targetItemId == 49318 // 上锁的库库尔坎初级宝箱
							) || (targetItemId == 49322 // 上锁的库库尔坎高级宝箱
							)) {
					...
					if (pc.isCrown() && (pc.getMapId() == 2000) && (pc.getX() == 32807) && (pc.getY() == 32773)) {
						...
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								L1MonsterInstance mob = (L1MonsterInstance) obj;
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 81323) { // 神官奇诺(新50级试炼怪-王族)
										found = true;
										break;
									}
								}
						...
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								L1MonsterInstance mob = (L1MonsterInstance) obj;
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 81324) { // 神官奇诺(新50级试炼怪-骑士)
										found = true;
										break;
									}
								}
						...
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								L1MonsterInstance mob = (L1MonsterInstance) obj;
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 81325) { // 神官奇诺(新50级试炼怪-妖精)
										found = true;
										break;
									}
								}
						...
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
								L1MonsterInstance mob = (L1MonsterInstance) obj;
								if (mob != null) {
									if (mob.getNpcTemplate().get_npcId() == 81326) { // 神官奇诺(新50级试炼怪-法师)
										found = true;
										break;
									}
								}
				...
				// 龙之钥匙
					...
					if ((locX != 0) && (locY != 0)) { // 各种传送卷轴
						...
						if (l1iteminstance.getCount() < 1) { // 不可能的？
			...
			// 种别：武器
				...
				if ((min != 0) && (min > pc.getLevel())) {
					...
					if ((pc.isCrown() && l1iteminstance.getItem().isUseRoyal()) // 王族
							|| (pc.isKnight() && l1iteminstance.getItem().isUseKnight()) // 骑士
							|| (pc.isElf() && l1iteminstance.getItem().isUseElf()) // 精灵
							|| (pc.isWizard() && l1iteminstance.getItem().isUseMage()) // 魔法师
							|| (pc.isDarkelf() && l1iteminstance.getItem().isUseDarkelf()) // 黑暗精灵
							|| (pc.isDragonKnight() && l1iteminstance.getItem().isUseDragonknight()) // 龙骑士
							|| (pc.isIllusionist() && l1iteminstance.getItem().isUseIllusionist()) // 幻术师
							) {
			...
			// 种别：防具
				...
				if ((pc.isCrown() && l1iteminstance.getItem().isUseRoyal()) // 王族
					...
					int min = ((L1Armor) l1iteminstance.getItem()).getMinLevel(); // 最低等级
					int max = ((L1Armor) l1iteminstance.getItem()).getMaxLevel(); // 最高等级
					if ((min != 0) && (min > pc.getLevel())) {
						pc.sendPackets(new S_ServerMessage(318, String.valueOf(min))); // 等级 %0以上才可使用此道具。
					}
					else if ((max != 0) && (max < pc.getLevel())) {
			...
			L1ItemDelay.onItemUse(client, l1iteminstance); // 项目开始延迟
		}
	}
	...
	private boolean usePolyScroll(L1PcInstance pc, int item_id, String s) {
		...
		if ((awakeSkillId == AWAKEN_ANTHARAS // 龙骑士魔法 (觉醒：安塔瑞斯)
			...
			) || (awakeSkillId == AWAKEN_VALAKAS // 龙骑士魔法 (觉醒：巴拉卡斯)
			)) {
	...
	private void UseArmor(L1PcInstance activeChar, L1ItemInstance armor) {
		...
		if (equipeSpace && !armor.isEquipped()) { // 使用した防具を装备していなくて、その装备个所が空いている场合（装着を试みる）
			...
			if ((type == 7) && (activeChar.getWeapon() != null)) { // シールドの场合、武器を装备していたら两手武器チェック
				if (activeChar.getWeapon().getItem().isTwohandedWeapon()) { // 双手武器
	...
	private void UseWeapon(L1PcInstance activeChar, L1ItemInstance weapon) {
		...
		if (weapon.getItemId() == 200002) { // 受诅咒的骰子匕首
	...
	private int doWandAction(L1PcInstance user, L1Object target) {
		...
		if (target instanceof L1PcInstance) {
			...
			if ((pc.hasSkillEffect(50) == true // 冰矛围篱
				) || (pc.hasSkillEffect(78) == true // 绝对屏障
				) || (pc.hasSkillEffect(157) == true // 大地屏障
				)) {
				return 0;
			}
	...
	private boolean writeLetter(int itemId, L1PcInstance pc, int letterCode, String letterReceiver, byte[] letterText) {
		...
		else if (itemId == 40731) { // 情人节卡片
			newItemId = 49022;
		}
		else if (itemId == 40732) { // 白色情人节卡片
			newItemId = 49024;
		}
	...
	private boolean writeClanLetter(int itemId, L1PcInstance pc, int letterCode, String letterReceiver, byte[] letterText) {
		...
		for (String element : memberName) {
			L1ItemInstance item = ItemTable.getInstance().createItem(49016); // 信纸
			if (item == null) {
				return false;
			}
			item.setCount(1);
	...
	private boolean withdrawPet(L1PcInstance pc, int itemObjectId) {
		...
		for (L1NpcInstance petNpc : pc.getPetList().values()) {
			if (petNpc instanceof L1PetInstance) {
				if (((L1PetInstance) petNpc).getItemObjId() == itemObjectId) { // 已经取出宠物
					return false;
				}
			}
	...
	private void useResolvent(L1PcInstance pc, L1ItemInstance item, L1ItemInstance resolvent) {
		...
		if ((item.getItem().getType2() == 1) || (item.getItem().getType2() == 2)) { // 武器・防具
			if (item.getEnchantLevel() != 0) { // 强化过的
	...
	private void useFurnitureItem(L1PcInstance pc, int itemId, int itemObjectId) {
		...
		for (L1Object l1object : L1World.getInstance().getObject()) {
			if (l1object instanceof L1FurnitureInstance) {
				...
				if (furniture.getItemObjId() == itemObjectId) { // 家具已经退出了
					isAppear = false;
					break;
				}
			}
