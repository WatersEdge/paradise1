...
public class S_Letter extends ServerBasePacket {
	...
	private void buildPacket(final L1ItemInstance item) {
		 ...
		 * writeH(item.get_gfxid()); } writeH(rs.getInt(2)); writeS(rs.getString(3)); writeS(rs.getString(4)); writeByte(rs.getBytes(7)); writeByte(rs.getBytes(8)); writeC(rs.getInt(6)); // テンプレ writeS(rs.getString(5)); // 日付 } } catch (SQLException e) { _log.log(Level.SEVERE,
		 * e.getLocalizedMessage(), e); } finally { SQLUtil.close(rs); SQLUtil.close(pstm); SQLUtil.close(con); }
		 */
	}
}
