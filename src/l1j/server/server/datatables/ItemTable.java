...
public class ItemTable {
	...
	private Map<Integer, L1Weapon> allWeapon() {
		...
		try {
			...
			_log.log(Level.SEVERE, new StringBuilder().append(weapon.getName()).append("(" + weapon.getItemId() + ")").append("无法加载。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
	...
	private Map<Integer, L1Armor> allArmor() {
		...
		try {
			...
			_log.log(Level.SEVERE, new StringBuilder().append(armor.getName()).append("(" + armor.getItemId() + ")").append("无法加载。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
