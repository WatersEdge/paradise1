...
public class C_LoginToServer extends ClientBasePacket {
	...
	public C_LoginToServer(final byte abyte0[], final ClientThread client) throws FileNotFoundException, Exception {
		...
		/** 初始能力加成 */
		final S_InitialAbilityGrowth AbilityGrowth = new S_InitialAbilityGrowth(pc);
		...
		final S_MapID s_mapid = new S_MapID(pc.getMapId(), pc.getMap().isUnderwater());
		...
		final S_CharTitle s_charTitle = new S_CharTitle(pc.getId(), pc.getTitle());
					...
					for (final L1War war : L1World.getInstance().getWarList()) {
						...
						if (ret) {
							...
							if (enemy_clan_name != null) {
								pc.sendPackets(new S_War(8, pc.getClanname(), enemy_clan_name)); // \f1目前你的血盟与 %0 血盟交战当中。
							}
							break;
						}
					}
		...
		if (pc.getPartnerId() != 0) {
			// 取得伴侣ID
			final L1PcInstance partner = (L1PcInstance) L1World.getInstance().findObject(pc.getPartnerId());
