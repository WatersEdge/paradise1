...
public class C_ChatWhisper extends ClientBasePacket {
	...
	public C_ChatWhisper(final byte abyte0[], final ClientThread client) throws Exception {
		...
		if (whisperTo.getExcludingList().contains(whisperFrom.getName())) {
			whisperFrom.sendPackets(new S_ServerMessage(117, whisperTo.getName())); // %0%s 断绝你的密语。
			return;
		}
		// 关闭密语
		if (!whisperTo.isCanWhisper()) {
			whisperFrom.sendPackets(new S_ServerMessage(205, whisperTo.getName())); // \f1%0%d 目前关闭悄悄话。
			return;
		}
