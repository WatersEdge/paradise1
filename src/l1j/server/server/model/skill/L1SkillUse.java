...
public class L1SkillUse {
	...
	private static class TargetStatus {
		...
		private L1Character _target = null;
		/** 计算器 */
		private boolean _isCalc = true; // ダメージや确率魔法の计算をする必要があるか？ (你需要计算魔法伤害和确率？)
	...
	public boolean checkUseSkill(L1PcInstance player, int skillid, int target_id, int x, int y, String message, int time, int type, L1Character attacker, int actid, int gfxid, int mpConsume) {
		...
		if (!(_target instanceof L1MonsterInstance) && _skill.getTarget().equals("attack") && (_user.getId() != target_id)) {
			_isPK = true; // 在目标怪物以外的攻击技能、其他场合与PK模式。
		}
	...
	private boolean isNormalSkillUsable() {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (pc.isParalyzed()) { // 麻痹・冻结状态
				return false;
			}
			if ((pc.isInvisble() || pc.isInvisDelay()) && !isInvisUsableSkill()) { // 隐身中无法使用技能
				return false;
			}
...
*/
		...
		if (cha instanceof L1NpcInstance) {
			...
			else if (hiddenStatus == L1NpcInstance.HIDDEN_STATUS_FLY) { // 对飞天的怪物无效
				return false;
			}
		}

		if (((_skill.getTargetTo() & L1Skills.TARGET_TO_PC) == L1Skills.TARGET_TO_PC // 目标PC
				)
		...
		if ((_calcType == PC_PC) && (cha instanceof L1PcInstance)) {
			if (((_skill.getTargetTo() & L1Skills.TARGET_TO_CLAN) == L1Skills.TARGET_TO_CLAN) && (((_player.getClanid() != 0 // 目标血盟
					) && (_player.getClanid() == ((L1PcInstance) cha).getClanid())) || _player.isGm())) {
				return true;
			}
			if (((_skill.getTargetTo() & L1Skills.TARGET_TO_PARTY) == L1Skills.TARGET_TO_PARTY) && (_player.getParty() // 目标组队
	...
	private void makeTargetList() {
		try {
			...
			if ((_skillId == LIGHTNING) || (_skillId == FREEZING_BREATH)) { // 极光雷电、寒冰喷吐 直线的范围
	...
	/** 精灵魔法的属性与使用者的属性不一致？*/ //（とりあえずの对处なので、对应できたら消去して下さい)
